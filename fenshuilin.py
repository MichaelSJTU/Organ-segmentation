...
# Step1. 加载图像
img = cv2.imread('/Users/jaymichael/Downloads/py_project/tooth_py/test1/1133.bmp')
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
# Step2.阈值分割，将图像分为黑白两部分
ret, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
cv2.imshow("thresh", thresh)
cv2.imwrite(savepath+'2.bmp',thresh)

# Step3. 对图像进行“开运算”，先腐蚀再膨胀
kernel = np.ones((3, 3), np.uint8)
opening = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel, iterations=1)
cv2.imshow("opening", opening)
cv2.imwrite(savepath+'3.bmp',opening)

# Step4. 对“开运算”的结果进行膨胀，得到大部分都是背景的区域
sure_bg = cv2.dilate(opening, kernel, iterations=1)
cv2.imshow("sure_bg", sure_bg)
cv2.imwrite(savepath+'4.bmp',sure_bg)
...
cv2.imwrite(savepath+'5.bmp',sure_fg)
...
cv2.imshow("result", unknow)
cv2.imwrite(savepath+'6.bmp',unknow)
...
cv2.imwrite(savepath+'1.bmp',img)
